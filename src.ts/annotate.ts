...
import evm from "./evm";
export const lookupTable = Object.keys(evm).reduce((r, byte) => {
  r[evm[byte][0]] = { pop: evm[byte][2], push: evm[byte][3], dataSize: evm[byte][4] };
  return r;
}, {});
  
export function extractLabel(v) {
  return v.split('(')[0];
}

export function annotateSegment(segment) {
  let ptr = 0;
  let vars = 0;
  let stack = [];
  let shallow = 0;
...
export function annotate(disasm) {
